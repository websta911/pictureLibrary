...
@app.route('/allSubscriptions', methods=['GET','POST'])
@is_logged_in
def allSubscriptions():	
    eves = db.session.query(Event).join(Subscription,Subscription.va == Event.eShort)
    sel = ''
    print(len(eves.all()))

    if request.method == 'POST':
        subs = db.session.query(Subscription).filter(Subscription.va == request.form['va'])
        sel = request.form['va']
    else:
        if (len(eves.all())) > 0 :
            subs = db.session.query(Subscription).filter(Subscription.va == eves[0].eShort)
            sel = eves[0].eShort
        else:
            subs = db.session.query(Subscription).all()

    return render_template('show_subs.html', **{
        'subs': subs,
        'eves': eves,
        'sel': sel })

@app.route('/deleteSubscriptions/<string:va>', methods=['POST'])
@is_logged_in
def deleteSubscriptions(va):
    delete_q = Subscription.__table__.delete().where(Subscription.va == va)
    db.session.execute(delete_q)
    db.session.commit()
    flash('deleted Subscriptions for {}'.format(va),'success')
    return redirect(url_for('allSubscriptions'))
@app.route("/upload")
@is_logged_in
def upload():
...
@app.route('/PBprint/<string:filename>')
@is_logged_in
def PBcups_print(filename):
    target = os.path.join(basedir,picture_basename)
    fullPath = os.path.join(target, filename)
    print("PBPrint-fullpath:", fullPath)
    CupsPrinting(fullPath)
    flash('Druckauftrag abgesendet.', 'success')
    return redirect('/')
...
def deleteEvent(id):
    ...
    event = remEv.event
	#test: delete subscriptions of event
    va = remEv.eShort
    delete_q = Subscription.__table__.delete().where(Subscription.va == va)
    db.session.execute(delete_q)
    #db.session.commit() #wahrscheinlich nur ein commit notwendig
    flash('deleted Subscriptions for {}'.format(va),'success')
    #test ende
